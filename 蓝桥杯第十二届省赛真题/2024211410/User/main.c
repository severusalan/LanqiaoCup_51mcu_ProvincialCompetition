...
unsigned char Seg_Disp_Mode;//数码管显示模式变量 0-温度显示界面 1-参数设置界面 2-DAC输出
float Temperature;//实时温度变量
unsigned char Temp_Disp = 25;//显示参数
unsigned char Temp_Ctrol = 25;//实时控制参数
...
void Seg_Proc()
{
	...
	switch(Seg_Disp_Mode)
	{
		case 0: //温度显示
			...
			Seg_Buf[6] = (unsigned char)(Temperature * 100) / 10 % 10;
...
void Timer1_Init(void)
{
	...
	TMOD &= 0x0F; // 设置定时器模式为16位定时器(16位自动重载)
	TL1 = 0x18;	  // 设置定时器初始值
	TH1 = 0xFC;	  // 设置定时器初始值
	TF1 = 0;	  // 清除TF1标志位
	TR1 = 1;	  // 启动定时器
